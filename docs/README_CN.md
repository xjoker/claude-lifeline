...
- **烧速趋势 `^` / `v`**——加速 (`^`) 或减速 (`v`) 箭头，样本不足时不显示。"出现 = 有把握"
- **Sonnet 子配额告警**——当 Sonnet 专属用量超速时，在 7d 块旁追加一个 `S:U/P%!` 块（否则隐藏）

### 烧速趋势工作原理

趋势箭头用两个 EWMA：短半衰期反映"当下烧速"，长半衰期代表"窗口平均烧速"。两者明显分歧时出现箭头。少于 4 个样本时不显示（避免新 session 误报）。历史按 `session_id` 分文件存在 `<claude_root>/claude-lifeline/history/`，多个 CC 窗口并发不会互相覆盖。
